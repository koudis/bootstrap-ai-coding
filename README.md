...
Primarily designed to work with Codium and Visual Studio Code but it works well only thru standard SSH session.
...
| `--host-network-off` | Use bridge networking with port mapping instead of host network mode |
...
```bash
# Default agents (claude-code, augment-code, build-resources)
bac <project-path>
